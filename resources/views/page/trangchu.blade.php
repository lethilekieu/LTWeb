						...
						<div class="beta-products-list">
							...
							<div class="row">
								...
								<div class="col-sm-3">
									...
									<div class="single-item-caption" id='id_{{$new->id}}'>
										...
										<div id='danhgia_{{$new->id}}'>	
											 <input type="number" min="1" max="5" value="3">&#9733
											 {{-- <input type="radio" value="1" name=diem_{{$new->id}}>1 &#9733 
											 <input type="radio" value="2"  name=diem_{{$new->id}}>2 &#9733
											 <input type="radio" value="3"  name=diem_{{$new->id}}>3 &#9733
											 <input type="radio" value="4"  name=diem_{{$new->id}}>4 &#9733
											 <input type="radio" value="5"  name=diem_{{$new->id}}>5 &#9733 --}}
											 <button onClick="F('{{$new->id}}')">Danh gia</button>
										</div>
											<div class="clearfix"></div>
										</div>
									</div>
									...
									<div class="single-item">
										...
										<div class="single-item-caption">
											...
											<div id='danhgia_{{$spkm->id}}'>	
												...
												<input type="radio" value="4"  name=diem_{{$new->id}}>4 &#9733
												<input type="radio" value="5"  name=diem_{{$new->id}}>5 &#9733 --}}
												<button onClick="F('{{$spkm->id}}')">Danh gia</button>
										   </div>
											<div class="clearfix"></div>
										</div>
									</div>
...
<script>
	...
	// n = jQuery('#id_' + id+' input[type=radio]:checked').val();
		jQuery.ajax(
			...
			url:'danhgia',
			type:'POST',
			data:"id="+id+"&diem="+n+"&_token={{csrf_token()}}",
			success:function(s)
			{
				console.log(s);
				if (s==1)
